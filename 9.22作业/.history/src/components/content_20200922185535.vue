...
export default {
  ...
  data() {
    return {
      list: JSON.parse(JSON.stringify(this.$store.state.login.list)),
      asideList: JSON.parse(JSON.stringify(this.$store.state.login.list))
    };
  },
  methods: {
    change() {
      this.asideList = JSON.parse(JSON.stringify(this.list));
    }
  }
};
</script>
