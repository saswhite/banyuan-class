...
// router.post('/checkPassword', (ctx, next) => {
//   const { password } = ctx.request.body;
//   let patten = /\w{8,15}/;
//   let flagP = patten.test(password);
//   ctx.response.body = {
//     flagP
//   }
// })
...
router.post('/regist', (ctx, next) => {
  const { name, password, decide } = ctx.request.body;
  let pattenP = /\w{8,15}/;
  let flagP = pattenP.test(password);
  let pattenN = /[a-zA-Z_-]{4,16}/;
  let flagN = pattenN.test(name);
  let flagD = false;
  if (password === decide) {
    flagD = true;
  }
  if (flagD && flagN && flagP) {
    ctx.response.body = {
      status: 'success'
    }
  }
